...
module RailsApp
  class CLI
    def self.start
      ...
      bundling = prompt.select("Choose your javascript bundling?", %w[importmap esbuild bun])
      styling = prompt.select("Choose your CSS framework?", %w[tailwind bootstrap none])
